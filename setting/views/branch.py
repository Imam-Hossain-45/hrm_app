...
from setting.forms import (BranchGeneralForm, PhysicalAddressForm, VirtualAddressForm, SocialLinkForm,
                           IdentificationForm)
...
def get_parent_item():
    if OrganizationalStructure.objects.filter(item='branch').exists():
        structure_object = OrganizationalStructure.objects.get(item='branch')
        if structure_object.parent_item:
            return str(structure_object.parent_item.item)
    return None


class BranchList(LoginRequiredMixin, PermissionMixin, ListView):
    """List of branches."""

    template_name = 'setting/branch/list.html'
    model = Branch
    permission_required = ['add_branch', 'update_branch', 'view_branch', 'delete_branch']
    ...
    def get_context_data(self, **kwargs):
        ...
        query_list = Branch.objects.filter(deleted=False).order_by('id')
        paginator = Paginator(query_list, 50)
        page = self.request.GET.get('page')
        context['branches'] = paginator.get_page(page)
        index = context['branches'].number - 1
...
class BranchCreate(LoginRequiredMixin, PermissionMixin, FormView):
    """Create and save a newly created branch."""

    permission_required = 'add_branch'
    template_name = 'setting/branch/create.html'
    form_class = BranchGeneralForm
    ...
    def get_context_data(self, **kwargs):
        ...
        print(context['physical_address_form'])
    ...
    def post(self, request, *args, **kwargs):
        ...
        context['org_items_list'] = get_organizational_structure()
        context['parent_item'] = get_parent_item()
        branch_general_form = self.form_class(request.POST, request.FILES)
        context['general_form'] = branch_general_form
        context['form_type'] = 'create'
        ...
        if branch_general_form.is_valid():
            branch = branch_general_form.save(commit=False)
            different_physical_address = branch_general_form.cleaned_data['different_physical_address']
            different_virtual_address = branch_general_form.cleaned_data['different_virtual_address']
            ...
            company = branch_general_form.cleaned_data['company']
            business_unit = branch_general_form.cleaned_data['business_unit']
            branch_in = branch_general_form.cleaned_data['branch']
            division = branch_general_form.cleaned_data['division']
            department = branch_general_form.cleaned_data['department']
            ...
            if different_physical_address:
                for physical_address_form in physical_address_form_set:
                    ...
                    BranchPhysicalAddress.objects.create(branch=branch, physical_address=physical_address)
                ...
                if "company".__eq__(belongs_to):
                    if CompanyPhysicalAddress.objects.filter(company=company,
                        ...
                        for address in company_physical_addresses:
                            ...
                            new_address = \
                                BranchPhysicalAddress.objects.create(branch=branch, inherited=True,
                    ...
                    if BranchPhysicalAddress.objects.filter(branch=branch_in,
                                                            physical_address__status='active').exists():
                        branch_physical_addresses = \
                            BranchPhysicalAddress.objects.filter(branch=branch_in, physical_address__status='active')
            ...
            if different_virtual_address:
                for phone_form in phone_form_set:
                    ...
                    if phone_form.cleaned_data.get('address'):
                        ...
                        phone.save()
                        BranchVirtualAddress.objects.create(branch=branch, virtual_address=phone)
                ...
                for email_form in email_form_set:
                    ...
                    if email_form.cleaned_data.get('address'):
                        ...
                        email.save()
                        BranchVirtualAddress.objects.create(branch=branch, virtual_address=email)
                ...
                for website_form in website_form_set:
                    ...
                    if website_form.cleaned_data.get('address'):
                        ...
                        website.save()
                        BranchVirtualAddress.objects.create(branch=branch, virtual_address=website)
                ...
                for fax_form in fax_form_set:
                    ...
                    if fax_form.cleaned_data.get('address'):
                        ...
                        fax.save()
                        BranchVirtualAddress.objects.create(branch=branch, virtual_address=fax)
                ...
                for social_link_form in social_link_form_set:
                    ...
                    if social_link_form.cleaned_data.get('link') and social_link_form.cleaned_data.get('type'):
                        ...
                        social_link.save()
                        BranchSocialLink.objects.create(branch=branch, social_link=social_link)
                ...
                if "company".__eq__(belongs_to):
                    if CompanyVirtualAddress.objects.filter(company=company, virtual_address__status='active').exists():
                        ...
                        for address in company_virtual_addresses:
                            ...
                            new_address = \
                                BranchVirtualAddress.objects.create(branch=branch, inherited=True,
                    ...
                    if CompanySocialLink.objects.filter(company=company, social_link__status='active').exists():
                        ...
                        for link in company_social_links:
                            ...
                            new_link = \
                                BranchSocialLink.objects.create(branch=branch, inherited=True,
                    ...
                    if BranchVirtualAddress.objects.filter(branch=branch_in, virtual_address__status='active').exists():
                        branch_virtual_addresses = \
                            BranchVirtualAddress.objects.filter(branch=branch_in, virtual_address__status='active')
                    ...
                    if BranchSocialLink.objects.filter(branch=branch_in, social_link__status='active').exists():
                        branch_social_links = \
                            BranchSocialLink.objects.filter(branch=branch_in, social_link__status='active')
                ...
                if parent_identification_form.my_is_valid():
                    ...
                    if parent_identification_form.is_valid():
                        if ((parent_identification_form.cleaned_data.get('title')) and
                                ...
                                identification.save()
                                BranchIdentification.objects.create(branch=branch, identification=identification)
                    ...
                    if "company".__eq__(belongs_to):
                        if CompanyIdentification.objects.filter(company=company, identification__title=title,
                            ...
                            if identification_qs.exists():
                                for identification in identification_qs:
                                    new_identification = \
                                        BranchIdentification.objects.create(
                                            branch=branch, inherited=True, identification=identification,
                        ...
                        if BusinessUnitIdentification.objects.filter(
                            ...
                            if identification_qs.exists():
                                for identification in identification_qs:
                                    new_identification = \
                                        BranchIdentification.objects.create(
                                            branch=branch, inherited_identification=identification, inherited=True,
                        ...
                        if BranchIdentification.objects.filter(
                            branch=branch_in, identification__title=title,
                            identification__short_description=short_description, identification__status='active'
                        ).exists():
                            identification_qs = \
                                Identification.objects.filter(
                                    branchidentification__branch=branch_in, title=title, status='active',
                            ...
                            if identification_qs.exists():
                                for identification in identification_qs:
                                    new_identification = \
                                        BranchIdentification.objects.create(
                                            branch=branch, inherited=True, identification=identification,
                                            inherited_identification=identification, inherited_branch=branch_in
                                        )
            ...
            return redirect(reverse_lazy('beehive_admin:setting:branch_list'))

        return render(request, self.template_name, {**context})


class BranchUpdate(LoginRequiredMixin, PermissionMixin, FormView):
    """Update the specified branch."""

    template_name = 'setting/branch/update.html'
    form_class = BranchGeneralForm
    permission_required = 'change_branch'

    def get_object(self, queryset=None):
        branch = Branch.objects.get(id=self.kwargs.get('pk', ''))
        return get_object_or_404(Branch, pk=branch.id)

    def get_physical_address_qs(self):
        physical_addresses = \
            PhysicalAddress.objects.filter(branchphysicaladdress__branch=self.get_object(),
                                           branchphysicaladdress__inherited=False, status='active')
        return physical_addresses

    def get_virtual_address_phone_qs(self):
        virtual_addresses_phone = \
            VirtualAddress.objects.filter(branchvirtualaddress__branch=self.get_object(),
                                          branchvirtualaddress__inherited=False, status='active',
                                          address_type='phone')
    ...
    def get_social_link_qs(self):
        social_link = SocialLink.objects.filter(branchsociallink__branch=self.get_object(),
                                                branchsociallink__inherited=False, status='active')
        return social_link

    def get_parent_identification_qs(self):
        identification = Identification.objects.filter(branchidentification__branch=self.get_object(),
    ...
    def get_context_data(self, **kwargs):
        ...
        initial = {
            ...
            'different_physical_address': branch.different_physical_address,
            'different_virtual_address': branch.different_virtual_address,
            'status': branch.status,
            'branch_start_date': branch.branch_start_date,
            'branch_end_date': branch.branch_end_date,
        }
        context['permissions'] = self.get_current_user_permission_list()
        context['org_items_list'] = get_organizational_structure()
        context['parent_item'] = get_parent_item()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if branch_general_form.is_valid():
            ...
            branch = Branch.objects.get(id=self.get_object().id)
            ...
            branch.name = branch_general_form.cleaned_data['name']
            branch.code = branch_general_form.cleaned_data['code']
            branch.description = branch_general_form.cleaned_data['description']
            branch.logo = branch_general_form.cleaned_data['logo']
            branch.different_physical_address = different_physical_address
            branch.different_virtual_address = different_virtual_address
            branch.status = branch_general_form.cleaned_data['status']
            branch.branch_start_date = branch_general_form.cleaned_data['branch_start_date']
            branch.branch_end_date = branch_general_form.cleaned_data['branch_end_date']
            branch.save()

            if different_physical_address:
                if PhysicalAddress.objects.filter(branchphysicaladdress__branch=branch,
                                                  branchphysicaladdress__inherited=True,
                                                  status='active').exists():
                    PhysicalAddress.objects.filter(branchphysicaladdress__branch=branch,
                                                   branchphysicaladdress__inherited=True,
                ...
                if PhysicalAddress.objects.filter(
                    branchphysicaladdress__branch=branch, branchphysicaladdress__inherited=False, status='active'
                ).exists():
                    PhysicalAddress.objects.filter(
                        branchphysicaladdress__branch=branch, branchphysicaladdress__inherited=False, status='active'
                    ).update(
                ...
                if "company".__eq__(belongs_to):
                    other_company_physical_addresses = \
                        PhysicalAddress.objects.filter(~Q(branchphysicaladdress__inherited_company=company),
                                                       branchphysicaladdress__branch=branch,
                                                       branchphysicaladdress__inherited=True, status='active')
                    ...
                    if CompanyPhysicalAddress.objects.filter(company=company,
                        ...
                        company_physical_addresses = \
                            CompanyPhysicalAddress.objects.filter(company=company, physical_address__status='active')
                        for address in company_physical_addresses:
                            if BranchPhysicalAddress.objects.filter(
                                branch=branch, inherited=True, physical_address__status='active',
                                ...
                                PhysicalAddress.objects.filter(
                                    branchphysicaladdress__branch=branch,
                                    branchphysicaladdress__inherited=True,
                                    branchphysicaladdress__inherited_physical_address=address.physical_address,
                                    branchphysicaladdress__physical_address__status='active',
                                    branchphysicaladdress__inherited_physical_address__status='active',
                                    branchphysicaladdress__inherited_company=company).\
                                    update(
                                ...
                                physical_address = address.physical_address
                                new_address = \
                                    BranchPhysicalAddress.objects.create(
                                        branch=branch, physical_address=physical_address, inherited=True,
                    ...
                    other_bu_physical_addresses = \
                        PhysicalAddress.objects.filter(~Q(branchphysicaladdress__inherited_business_unit=business_unit),
                                                       branchphysicaladdress__branch=branch,
                                                       branchphysicaladdress__inherited=True, status='active')
                        ...
                        for address in business_unit_physical_addresses:
                            if BranchPhysicalAddress.objects.filter(
                                ...
                                PhysicalAddress.objects.filter(
                                    branchphysicaladdress__branch=branch,
                                    branchphysicaladdress__inherited=True,
                                    branchphysicaladdress__inherited_physical_address=address,
                                    branchphysicaladdress__inherited_physical_address__status='active',
                                    branchphysicaladdress__inherited_business_unit=business_unit).\
                                    update(
                                    title=address.physical_address.title,
                    ...
                    other_branch_physical_addresses = \
                        PhysicalAddress.objects.filter(~Q(branchphysicaladdress__inherited_branch=branch_in),
                                                       branchphysicaladdress__branch=branch,
                                                       branchphysicaladdress__inherited=True, status='active')
                    ...
                    if BranchPhysicalAddress.objects.filter(branch=branch_in,
                        ...
                        for address in branch_physical_addresses:
                            if BranchPhysicalAddress.objects.filter(
                                branch=branch, inherited=True, physical_address__status='active',
                                inherited_physical_address=address.physical_address,
                                inherited_physical_address__status='active', inherited_branch=branch_in
                            ).exists():
                                PhysicalAddress.objects.filter(
                                    branchphysicaladdress__branch=branch,
                                    branchphysicaladdress__inherited=True,
                                    branchphysicaladdress__inherited_physical_address=address,
                                    branchphysicaladdress__inherited_physical_address__status='active',
                                    branchphysicaladdress__inherited_branch=branch_in).\
                                    update(
                                    title=address.physical_address.title,
                                    address_line_1=address.address_line_1,
                                    address_line_2=address.address_line_2,
                                ...
                                new_address = \
                                    BranchPhysicalAddress.objects.create(
                                        branch=branch, physical_address=physical_address, inherited=True,
                                        inherited_physical_address=physical_address, inherited_branch=branch_in
                                    )
                    ...
                    other_division_physical_addresses = \
                        PhysicalAddress.objects.filter(~Q(branchphysicaladdress__inherited_division=division),
                                                       branchphysicaladdress__branch=branch,
                                                       branchphysicaladdress__inherited=True, status='active')
                    ...
                    if DivisionPhysicalAddress.objects.filter(division=division,
                        ...
                        for address in division_physical_addresses:
                            if BranchPhysicalAddress.objects.filter(
                                ...
                                PhysicalAddress.objects.filter(
                                    branchphysicaladdress__branch=branch, branchphysicaladdress__inherited=True,
                                    branchphysicaladdress__inherited_physical_address=address,
                                    branchphysicaladdress__inherited_physical_address__status='active',
                                    branchphysicaladdress__inherited_division=division).\
                                    update(
                                    title=address.physical_address.title,
                                    address_line_1=address.address_line_1,
                                    address_line_2=address.address_line_2,
                    ...
                    other_department_physical_addresses = \
                        PhysicalAddress.objects.filter(~Q(branchphysicaladdress__inherited_department=department),
                                                       branchphysicaladdress__branch=branch,
                                                       branchphysicaladdress__inherited=True, status='active')
                        ...
                        for address in department_physical_addresses:
                            if BranchPhysicalAddress.objects.filter(
                                ...
                                PhysicalAddress.objects.filter(
                                    ...
                                    branchphysicaladdress__inherited_physical_address=address,
                                    branchphysicaladdress__inherited_physical_address__status='active',
                                    branchphysicaladdress__inherited_department=department).\
                                    update(
                                    title=address.physical_address.title,
                                    address_line_1=address.address_line_1,
                                    address_line_2=address.address_line_2,
            ...
            if different_virtual_address:
                if VirtualAddress.objects.filter(branchvirtualaddress__branch=branch,
                                                 branchvirtualaddress__inherited=True, status='active').exists():
                    VirtualAddress.objects.filter(branchvirtualaddress__branch=branch,
                                                  branchvirtualaddress__inherited=True, status='active').update(
                        status='inactive',
                        address_end_date=datetime.now()
                    )
                ...
                if VirtualAddress.objects.filter(branchvirtualaddress__branch=branch,
                                                 branchvirtualaddress__inherited=False, status='active').exists():
                    VirtualAddress.objects.filter(branchvirtualaddress__branch=branch,
                                                  branchvirtualaddress__inherited=False, status='active').update(
                        status='inactive',
                        address_end_date=datetime.now()
                    )

                if SocialLink.objects.filter(branchsociallink__branch=branch,
                                             branchsociallink__inherited=False, status='active').exists():
                    SocialLink.objects.filter(branchsociallink__branch=branch,
                                              branchsociallink__inherited=False, status='active').update(
                        status='inactive',
                        link_end_date=datetime.now()
                    )

                if "company".__eq__(belongs_to):
                    if VirtualAddress.objects.filter(~Q(branchvirtualaddress__inherited_company=company),
                                                     branchvirtualaddress__branch=branch,
                                                     branchvirtualaddress__inherited=True, status='active').exists():
                        VirtualAddress.objects.filter(~Q(branchvirtualaddress__inherited_company=company),
                                                      branchvirtualaddress__branch=branch,
                                                      branchvirtualaddress__inherited=True, status='active').update(
                            status='inactive',
                            address_end_date=datetime.now()
                    ...
                    if CompanyVirtualAddress.objects.filter(company=company, virtual_address__status='active').exists():
                        company_virtual_addresses = \
                            CompanyVirtualAddress.objects.filter(company=company, virtual_address__status='active')
                        for address in company_virtual_addresses:
                            if BranchVirtualAddress.objects.filter(
                                branch=branch, inherited=True, virtual_address__status='active',
                                inherited_virtual_address=address.virtual_address,
                                inherited_virtual_address__status='active', inherited_company=company
                            ).exists():
                                VirtualAddress.objects.filter(
                                    branchvirtualaddress__branch=branch, branchvirtualaddress__inherited=True,
                                    branchvirtualaddress__inherited_virtual_address=address.virtual_address,
                                    branchvirtualaddress__inherited_virtual_address__status='active',
                                    branchvirtualaddress__virtual_address__status='active',
                                    branchvirtualaddress__inherited_company=company).\
                                    update(
                                ...
                                virtual_address = address.virtual_address
                                new_address = \
                                    BranchVirtualAddress.objects.create(
                                        branch=branch, inherited=True, virtual_address=virtual_address,
                    ...
                    if CompanySocialLink.objects.filter(company=company, social_link__status='active').exists():
                        company_social_links = \
                            CompanySocialLink.objects.filter(company=company, social_link__status='active')
                        for link in company_social_links:
                            if BranchSocialLink.objects.filter(
                                branch=branch, inherited=True, inherited_social_link=link.social_link,
                                inherited_social_link__status='active', social_link__status='active',
                                inherited_company=company
                            ).exists():
                                SocialLink.objects.filter(
                                    branchsociallink__branch=branch, branchsociallink__inherited=True,
                                    branchsociallink__inherited_social_link=link.social_link,
                                    branchsociallink__inherited_social_link__status='active',
                                    branchsociallink__social_link__status='active',
                                    branchsociallink__inherited_company=company). \
                                    update(
                                ...
                                social_link = link.social_link
                                new_link = \
                                    BranchSocialLink.objects.create(
                                        branch=branch, inherited=True, social_link=social_link,
                    ...
                    if VirtualAddress.objects.filter(~Q(branchvirtualaddress__inherited_business_unit=business_unit),
                                                     branchvirtualaddress__branch=branch,
                                                     branchvirtualaddress__inherited=True, status='active'
                                                     ).exists():
                        VirtualAddress.objects.filter(~Q(branchvirtualaddress__inherited_business_unit=business_unit),
                                                      branchvirtualaddress__branch=branch,
                                                      branchvirtualaddress__inherited=True, status='active'
                                                      ).update(
                            status='inactive',
                    ...
                    if BusinessUnitVirtualAddress.objects.filter(business_unit=business_unit,
                        ...
                        for address in business_unit_virtual_addresses:
                            if BranchVirtualAddress.objects.filter(
                                ...
                                VirtualAddress.objects.filter(
                                    branchvirtualaddress__branch=branch,
                                    branchvirtualaddress__inherited=True,
                                    branchvirtualaddress__inherited_virtual_address=address.virtual_address,
                                    branchvirtualaddress__inherited_virtual_address__status='active',
                                    branchvirtualaddress__virtual_address__status='active',
                                    branchvirtualaddress__inherited_business_unit=business_unit). \
                                    update(
                                    address_type=address.virtual_address.address_type,
                                    address=address.virtual_address.address,
                                    description=address.virtual_address.description
                        ...
                        for link in business_unit_social_links:
                            if BranchSocialLink.objects.filter(
                                ...
                                SocialLink.objects.filter(
                                    ...
                                    branchsociallink__inherited_social_link__status='active',
                                    branchsociallink__social_link__status='active',
                                    branchsociallink__inherited_business_unit=business_unit). \
                                    update(
                                    type=link.social_link.type,
                                    link=link.social_link.link,
                                    description=link.social_link.description
                    ...
                    if VirtualAddress.objects.filter(~Q(branchvirtualaddress__inherited_branch=branch_in),
                                                     branchvirtualaddress__branch=branch,
                                                     branchvirtualaddress__inherited=True, status='active').exists():
                        VirtualAddress.objects.filter(~Q(branchvirtualaddress__inherited_branch=branch_in),
                                                      branchvirtualaddress__branch=branch,
                                                      branchvirtualaddress__inherited=True, status='active').update(
                            status='inactive',
                            address_end_date=datetime.now()
                    ...
                    if BranchVirtualAddress.objects.filter(branch=branch_in, virtual_address__status='active').exists():
                        ...
                        for address in branch_virtual_addresses:
                            if BranchVirtualAddress.objects.filter(
                                branch=branch, inherited=True, virtual_address__status='active',
                                inherited_virtual_address=address.virtual_address,
                                inherited_virtual_address__status='active', inherited_branch=branch_in
                            ).exists():
                                VirtualAddress.objects.filter(
                                    branchvirtualaddress__branch=branch,
                                    branchvirtualaddress__inherited=True,
                                    ...
                                    branchvirtualaddress__inherited_virtual_address__status='active',
                                    branchvirtualaddress__virtual_address__status='active',
                                    branchvirtualaddress__inherited_branch=branch_in
                                ).update(
                                    address_type=address.virtual_address.address_type,
                                    address=address.virtual_address.address,
                                    description=address.virtual_address.description
                                ...
                                new_address = \
                                    BranchVirtualAddress.objects.create(
                                        branch=branch, inherited=True, virtual_address=virtual_address,
                                        inherited_virtual_address=virtual_address, inherited_branch=branch_in)
                    ...
                    if BranchSocialLink.objects.filter(branch=branch_in, social_link__status='active').exists():
                        ...
                        for link in branch_social_links:
                            if BranchSocialLink.objects.filter(
                                ...
                                SocialLink.objects.filter(
                                    branchsociallink__branch=branch,
                                    branchsociallink__inherited=True,
                                    branchsociallink__inherited_social_link=link.social_link,
                                    branchsociallink__inherited_social_link__status='active',
                                    branchsociallink__social_link__status='active',
                                    branchsociallink__inherited_branch=branch_in
                                ).update(
                                    type=link.social_link.type,
                                    link=link.social_link.link,
                                    description=link.social_link.description
                                ...
                                new_link = \
                                    BranchSocialLink.objects.create(
                                        branch=branch, inherited=True, social_link=social_link,
                                        inherited_social_link=social_link, inherited_branch=branch_in)
                    ...
                    if VirtualAddress.objects.filter(~Q(branchvirtualaddress__inherited_division=division),
                                                     branchvirtualaddress__branch=branch,
                                                     branchvirtualaddress__inherited=True, status='active'
                                                     ).exists():
                        VirtualAddress.objects.filter(~Q(branchvirtualaddress__inherited_division=division),
                                                      branchvirtualaddress__branch=branch,
                                                      branchvirtualaddress__inherited=True, status='active'
                                                      ).update(
                            status='inactive',
                        ...
                        for address in division_virtual_addresses:
                            if BranchVirtualAddress.objects.filter(
                                ...
                                VirtualAddress.objects.filter(
                                    ...
                                    branchvirtualaddress__inherited_virtual_address__status='active',
                                    branchvirtualaddress__virtual_address__status='active',
                                    branchvirtualaddress__inherited_division=division
                                ).update(
                                    address_type=address.virtual_address.address_type,
                                    address=address.virtual_address.address,
                                    description=address.virtual_address.description
                        ...
                        for link in division_social_links:
                            if BranchSocialLink.objects.filter(
                                ...
                                SocialLink.objects.filter(
                                    ...
                                    branchsociallink__inherited_social_link__status='active',
                                    branchsociallink__social_link__status='active',
                                    branchsociallink__inherited_division=division). \
                                    update(
                                    type=link.social_link.type,
                                    link=link.social_link.link,
                                    description=link.social_link.description
                    ...
                    if VirtualAddress.objects.filter(~Q(branchvirtualaddress__inherited_department=department),
                                                     branchvirtualaddress__branch=branch,
                                                     branchvirtualaddress__inherited=True, status='active'
                                                     ).exists():
                        VirtualAddress.objects.filter(~Q(branchvirtualaddress__inherited_department=department),
                                                      branchvirtualaddress__branch=branch,
                                                      branchvirtualaddress__inherited=True, status='active'
                                                      ).update(
                            status='inactive',
                        ...
                        for address in department_virtual_addresses:
                            if BranchVirtualAddress.objects.filter(
                                ...
                                VirtualAddress.objects.filter(
                                    ...
                                    branchvirtualaddress__inherited_virtual_address__status='active',
                                    branchvirtualaddress__virtual_address__status='active',
                                    branchvirtualaddress__inherited_department=department
                                ).update(
                                    address_type=address.virtual_address.address_type,
                                    address=address.virtual_address.address,
                                    description=address.virtual_address.description
                        ...
                        for link in department_social_links:
                            if BranchSocialLink.objects.filter(
                                ...
                                SocialLink.objects.filter(
                                    ...
                                    branchsociallink__inherited_social_link__status='active',
                                    branchsociallink__social_link__status='active',
                                    branchsociallink__inherited_department=department). \
                                    update(
                                    type=link.social_link.type,
                                    link=link.social_link.link,
                                    description=link.social_link.description
                ...
                if title and short_description:
                    if parent_identification_form.my_is_valid():
                        ...
                        if Identification.objects.filter(
                            branchidentification__branch=branch, different_from_parent=False,
                            ...
                            if parent_identification_form.is_valid():
                                if ((parent_identification_form.cleaned_data.get('title')) and
                                    ...
                                    current_identification = Identification.objects.filter(
                                        branchidentification__branch=branch, status='active',
                                    ...
                                    if current_identification.exists():
                                        ...
                                        BranchIdentification.objects.create(branch=branch,
                                                                            identification=new_identification)

                    else:
                        if parent_identification_form.is_valid():
                            parent_identification_form.save(commit=False)
                            if Identification.objects.filter(branchidentification__branch=branch,
                            ...
                            if "company".__eq__(belongs_to):
                                if Identification.objects.filter(~Q(branchidentification__inherited_company=
                                                                    company),
                                                                 branchidentification__branch=branch,
                                                                 branchidentification__inherited=True,
                                                                 short_description=short_description, title=title,
                                                                 different_from_parent=True, status='active').exists():
                                    Identification.objects.filter(~Q(branchidentification__inherited_company=
                                                                     company),
                                                                  branchidentification__branch=branch,
                                                                  branchidentification__inherited=True,
                                                                  short_description=short_description, title=title,
                                ...
                                existing_identification = \
                                    Identification.objects.filter(
                                        branchidentification__branch=branch, title=title,
                                        branchidentification__inherited_company=company,
                                        short_description=short_description, status='active')
                                ...
                                if Identification.objects.filter(~Q(branchidentification__inherited_business_unit=
                                                                    business_unit),
                                                                 branchidentification__branch=branch,
                                                                 branchidentification__inherited=True,
                                                                 different_from_parent=True, status='active').exists():
                                    Identification.objects.filter(~Q(
                                        branchidentification__inherited_business_unit=business_unit),
                                                                  branchidentification__branch=branch,
                                                                  branchidentification__inherited=True,
                                                                  different_from_parent=True, status='active').update(
                                        status='inactive',
                                ...
                                existing_identification = \
                                    Identification.objects.filter(
                                        branchidentification__branch=branch, title=title,
                                        branchidentification__identification=
                                ...
                                if Identification.objects.filter(~Q(branchidentification__inherited_branch=
                                                                    branch_in),
                                                                 branchidentification__branch=branch,
                                                                 branchidentification__inherited=True,
                                                                 different_from_parent=True, status='active').exists():
                                    Identification.objects.filter(~Q(
                                        branchidentification__inherited_branch=branch_in),
                                                                  branchidentification__branch=branch,
                                                                  branchidentification__inherited=True,
                                                                  different_from_parent=True, status='active').update(
                                        status='inactive',
                                ...
                                if existing_identification.exists():
                                    ...
                                    identification_qs = \
                                        Identification.objects.filter(branchidentification__branch=branch_in,
                                                                      title=title, short_description=short_description)
                                    if identification_qs.exists():
                                        for identification in identification_qs:
                                            new_identification = \
                                ...
                                if Identification.objects.filter(~Q(branchidentification__inherited_division=
                                                                    division),
                                                                 branchidentification__branch=branch,
                                                                 branchidentification__inherited=True,
                                                                 different_from_parent=True, status='active').exists():
                                    Identification.objects.filter(~Q(branchidentification__inherited_division=
                                                                     division),
                                                                  branchidentification__branch=branch,
                                                                  branchidentification__inherited=True,
                                                                  different_from_parent=True, status='active').update(
                                ...
                                if Identification.objects.filter(~Q(branchidentification__inherited_department=
                                                                    department),
                                                                 branchidentification__branch=branch,
                                                                 branchidentification__inherited=True,
                                                                 different_from_parent=True, status='active').exists():
                                    Identification.objects.filter(~Q(branchidentification__inherited_department=
                                                                     department),
                                                                  branchidentification__branch=branch,
                                                                  branchidentification__inherited=True,
                                                                  different_from_parent=True, status='active').update(
...
class BranchDelete(LoginRequiredMixin, PermissionMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        branch_object = self.get_object()
        branch = Branch.objects.get(id=branch_object.id)
        branch.deleted = True
        branch.status = 'inactive'
        branch.save()

        branch_physical_addresses = \
            PhysicalAddress.objects.filter(branchphysicaladdress__branch=branch, status='active')

        if branch_physical_addresses.exists():
            for address in branch_physical_addresses:
        ...
        branch_virtual_addresses = \
            VirtualAddress.objects.filter(branchvirtualaddress__branch=branch, status='active')

        if branch_virtual_addresses.exists():
            for address in branch_virtual_addresses:
                address.status = 'inactive'
                address.address_end_date = datetime.now()
                address.save()
        ...
        branch_social_links = \
            SocialLink.objects.filter(branchsociallink__branch=branch, status='active')

        if branch_social_links.exists():
            for link in branch_social_links:
        ...
        branch_identifications = \
            Identification.objects.filter(branchidentification__branch=branch, status='active')

        if branch_identifications.exists():
            for identification in branch_identifications:
